...
// Replace Bank Reconciliations with the new chart
const bankRecStart = "{/* Bank Feed Quick Actions */}";
const bankRecRegex = /\{\/\* Bank Feed Quick Actions \*\/\}[\s\S]*?<\/div>\s*<\/div>/;

text = text.replace(bankRecRegex, chartUI);
