...
export function ReceiptScanner({ onScanComplete, onClose }: ReceiptScannerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-ink-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
      <div className="bg-paper-100 rounded-lg shadow-2xl w-full max-w-lg overflow-hidden border border-ink-900/10">
        ...
        {!isScanning && (
          <div className="p-4 bg-paper-100 border-t border-ink-900/10 flex flex-col space-y-3">
            <button 
              onClick={captureImage} 
              disabled={!stream}
              className="w-full bg-focus-blue-500 text-white  font-medium py-3 rounded-sm hover:bg-focus-blue-600 transition-colors disabled:opacity-50"
            >
              Capture Receipt
            </button>
            <div className="relative">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-ink-900/10"></div></div>
              <div className="relative flex justify-center text-sm"><span className="bg-paper-100 px-2 text-slate-500">Or</span></div>
            </div>
            <label className="w-full cursor-pointer bg-paper-100 border border-ink-900/20 text-ink-900 font-medium py-2 rounded-sm hover:bg-paper-50 transition-colors flex items-center justify-center">
