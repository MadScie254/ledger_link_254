...
export function Header() {
  ...
  const { setCommandPaletteOpen, activeView, setActiveView, activeCompany, organizations, setActiveCompany, setCurrentOrgId, setDisplayCurrency } = useAppStore();
        ...
        <div className="flex flex-1 items-center space-x-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-2 px-2.5 py-1.5 rounded-sm border border-ink-900/15 bg-white dark:bg-[#111827] text-xs font-semibold text-ink-900 hover:bg-paper-50 transition-colors shadow-2xs"
            >
              ...
              <span className="font-mono text-[10px] px-1 py-0.2 bg-paper-100 dark:bg-ink-900/50 rounded text-slate-500 font-bold">
            ...
            {isCompanyDropdownOpen && (
              <div className="absolute left-0 mt-1.5 w-64 bg-white dark:bg-[#111827] border border-ink-900/15 rounded-sm shadow-lg py-1.5 z-50 animate-in fade-in">
          ...
          <div className="hidden md:flex max-w-md w-full cursor-text" onClick={() => setCommandPaletteOpen(true)}>
            ...
            <div className="relative w-full">
              ...
              <div
                className="block w-full pl-9 pr-3 py-1.5 border border-ink-900/10 rounded-sm leading-5 bg-white dark:bg-[#111827] text-slate-500 text-xs hover:border-focus-blue-500 transition-colors flex items-center justify-between"
              >
                <span>Search (Cmd+K)</span>
                <kbd className="hidden sm:inline-block px-1.5 py-0.5 text-[10px] font-mono bg-paper-100 dark:bg-ink-900/40 border border-ink-900/10 rounded text-slate-400">
                  ⌘K
                </kbd>
              </div>
            </div>
        ...
        <div className="ml-4 flex items-center md:ml-6 space-x-3">
          ...
          <div className="relative">
            <button
              ...
              className="bg-ink-900 text-white dark:text-slate-900 px-3 py-1.5 rounded-sm text-xs font-medium hover:bg-ink-900/90 transition-colors flex items-center space-x-1.5 shadow-xs"
            >
