...
var pkceVerifierSlotKey = /* @__PURE__ */ __name((storageKey, flowId) => `${storageKey}-flow-${flowId}-code-verifier`, "pkceVerifierSlotKey");
var pkceFlowIndexKey = /* @__PURE__ */ __name((storageKey) => `${storageKey}-flows-code-verifier`, "pkceFlowIndexKey");
...
var isNewApiKey = /* @__PURE__ */ __name((key) => key.startsWith("sb_publishable_") || key.startsWith("sb_secret_"), "isNewApiKey");
var TEMP_KEY_PREFIX = "sb_temp_";
var warnedKeySubtypes = /* @__PURE__ */ new Set();
var checkApiKeyFormat = /* @__PURE__ */ __name((key) => {
...
var fetchWithAuth = /* @__PURE__ */ __name((supabaseKey, supabaseUrl2, getAccessToken, customFetch, tracePropagationOptions, options) => {
...
var SupabaseClient = class {
  ...
  */
  constructor(supabaseUrl2, supabaseKey, options) {
    ...
    this.fetch = fetchWithAuth(supabaseKey, supabaseUrl2, this._getSessionToken.bind(this), settings.global.fetch, settings.tracePropagation);
    this.functionsFetch = fetchWithAuth(supabaseKey, supabaseUrl2, this._getSessionToken.bind(this), settings.global.fetch, settings.tracePropagation, { omitApiKeyAsBearer: true });
...
var getSupabase = /* @__PURE__ */ __name(() => {
...
var AuditService = class {
  ...
  static async getLogs(orgId, maxResults = 50) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("audit_logs").select("*").eq("org_id", orgId).order("timestamp", { ascending: false }).limit(maxResults);
...
var AccountService = class {
  ...
  static async createAccount(input) {
    const supabase = getSupabase();
    const { data: existing, error: searchError } = await supabase.from("accounts").select("id").eq("org_id", input.orgId).eq("code", input.code).limit(1);
    if (searchError) throw searchError;
    if (existing && existing.length > 0) {
      throw new Error(`Account code ${input.code} already exists.`);
    }
    const { data: newAccount, error: insertError } = await supabase.from("accounts").insert({
      org_id: input.orgId,
      code: input.code,
      name: input.name,
      type: input.type,
  ...
  static async getAccountByCode(orgId, code) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("accounts").select("*").eq("org_id", orgId).eq("code", code).limit(1).single();
    if (error && error.code !== "PGRST116") return null;
    return data;
  }
  static async bulkCreateAccounts(orgId, accounts, userId) {
    let success = 0;
    let failed = 0;
    const accountIds = [];
  ...
  static async bulkDeleteAccounts(orgId, accountIds, userId) {
    const supabase = getSupabase();
    const { error } = await supabase.from("accounts").delete().eq("org_id", orgId).in("id", accountIds);
    if (error) throw error;
    if (userId) {
  ...
  static async getAccounts(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("accounts").select("*").eq("org_id", orgId).order("code");
    if (error) throw error;
    const accounts = data || [];
   ...
   */
  static async getAccountBalances(orgId) {
    const supabase = getSupabase();
    const { data: lines, error } = await supabase.from("journal_lines").select("debit, credit, account:accounts!inner(id, type, org_id)").eq("accounts.org_id", orgId);
    if (error) throw error;
    const balances = /* @__PURE__ */ new Map();
    const creditNormalTypes = /* @__PURE__ */ new Set(["LIABILITY", "EQUITY", "INCOME"]);
...
var CustomerService = class {
  ...
  static async getCustomers(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("customers").select("*").eq("org_id", orgId).order("display_name");
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async createCustomer(orgId, input) {
    const supabase = getSupabase();
  ...
  static async updateCustomer(orgId, id, input) {
    const supabase = getSupabase();
    const updateData = {};
    if (input.displayName !== void 0) updateData.display_name = input.displayName;
    if (input.legalName !== void 0) updateData.legal_name = input.legalName;
    if (input.customerType !== void 0) updateData.customer_type = input.customerType;
    if (input.contactPerson !== void 0) updateData.contact_person = input.contactPerson;
    if (input.email !== void 0) updateData.email = input.email;
    if (input.phone !== void 0) updateData.phone = input.phone;
    if (input.kraPin !== void 0) updateData.kra_pin = input.kraPin;
    if (input.paymentTerms !== void 0) updateData.payment_terms = input.paymentTerms;
    if (input.creditLimitCents !== void 0) updateData.credit_limit_cents = input.creditLimitCents;
    if (input.currency !== void 0) updateData.currency = input.currency;
    if (input.discountPercent !== void 0) updateData.discount_percent = input.discountPercent;
    if (input.priceTier !== void 0) updateData.price_tier = input.priceTier;
    if (input.billingAddress !== void 0) updateData.billing_address = input.billingAddress;
    if (input.shippingAddress !== void 0) updateData.shipping_address = input.shippingAddress;
    if (input.city !== void 0) updateData.city = input.city;
    if (input.postalCode !== void 0) updateData.postal_code = input.postalCode;
    if (input.country !== void 0) updateData.country = input.country;
    if (input.notes !== void 0) updateData.notes = input.notes;
    const { error } = await supabase.from("customers").update(updateData).eq("id", id).eq("org_id", orgId);
    if (error) throw error;
  }
};

// src/server/types.ts
var LedgerEngineError = class extends Error {
  static {
    __name(this, "LedgerEngineError");
  }
  constructor(message) {
    super(message);
...
var LedgerService = class {
   ...
   */
  static async postJournalEntry(input) {
    const supabase = getSupabase();
  ...
  static async getJournalEntries(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("journal_entries").select(`
        *,
        lines:journal_lines(*)
      `).eq("org_id", orgId).order("posted_at", { ascending: false });
    if (error) throw error;
...
// src/server/etims.ts
var EtimsService = class {
  static {
    __name(this, "EtimsService");
  }
  static async submitInvoice(orgId, invoiceId, invoiceData) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("etims_submissions").insert({
      org_id: orgId,
...
var InvoiceService = class {
  ...
  static async createInvoice(input) {
    ...
    const journalLines = [
      {
        ...
        debit: totalCents,
        credit: 0,
        description: currency !== "KES" ? `Invoice A/R (${currency} ${(totalForeignCents / 100).toFixed(2)} @ ${exchangeRate})` : void 0,
        entityType: "CUSTOMER",
    ...
    try {
      ...
      const { error: cleanupError } = await supabase.from("invoices").delete().eq("id", invoiceRefId).eq("org_id", input.orgId);
      if (cleanupError) {
  ...
  static async voidInvoice(orgId, invoiceId, voidedBy) {
    const supabase = getSupabase();
    const { data: invoice, error: invoiceError } = await supabase.from("invoices").select("*").eq("id", invoiceId).eq("org_id", orgId).single();
    ...
    if (originalEntry) {
      ...
      await LedgerService.postJournalEntry({
        ...
        sourceType: "INVOICE",
        sourceId: invoiceId,
        // Reference the same invoice ID
        referenceNo: `VOID-${invoice.invoice_number}`,
        createdBy: voidedBy,
        lines: reversingLines
      });
    }
    const { error: updateError } = await supabase.from("invoices").update({ status: "VOID" }).eq("id", invoiceId);
    if (updateError) throw updateError;
  }
  static async updateInvoice(orgId, id, input) {
    const supabase = getSupabase();
    if (input.status !== void 0) {
      throw new Error("Invoice status must be changed through a dedicated payment or void workflow.");
    }
    const updateData = {};
    if (input.dueDate !== void 0) updateData.due_date = input.dueDate;
    if (input.notes !== void 0) updateData.notes = input.notes;
    if (Object.keys(updateData).length > 0) {
      const { error } = await supabase.from("invoices").update(updateData).eq("id", id).eq("org_id", orgId);
      if (error) throw error;
    }
  }
};
...
var BillService = class {
  static {
    __name(this, "BillService");
  }
  static async getBills(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("bills").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async createBill(input) {
    ...
    const journalLines = [
      {
        ...
        debit: 0,
        credit: totalCents,
        description: currency !== "KES" ? `Bill A/P (${currency} ${(totalForeignCents / 100).toFixed(2)} @ ${exchangeRate})` : void 0,
        entityType: "VENDOR",
    ...
    try {
      ...
      const { error: cleanupError } = await supabase.from("bills").delete().eq("id", billRefId).eq("org_id", input.orgId);
      if (cleanupError) {
  ...
  static async voidBill(orgId, billId, voidedBy) {
    const supabase = getSupabase();
    const { data: bill, error: billError } = await supabase.from("bills").select("*").eq("id", billId).eq("org_id", orgId).single();
    if (billError || !bill) {
    ...
    const { error: updateError } = await supabase.from("bills").update({ status: "VOID" }).eq("id", billId);
    if (updateError) throw updateError;
  }
  /**
   ...
   */
  static async recordPayment(orgId, billId, paymentDate, paidBy) {
    const supabase = getSupabase();
    const { data: bill, error: billError } = await supabase.from("bills").select("*").eq("id", billId).eq("org_id", orgId).single();
    ...
    const { error: updateError } = await supabase.from("bills").update({ status: "PAID", amount_due_cents: 0 }).eq("id", billId).eq("org_id", orgId);
    if (updateError) throw updateError;
  }
  static async recordBatchPayment(orgId, billIds, paymentDate, paidBy) {
    let paid = 0;
    let failed = 0;
  ...
  static async updateBill(orgId, id, input) {
    const supabase = getSupabase();
    if (input.status !== void 0) {
    ...
    if (input.notes !== void 0) updateData.notes = input.notes;
    if (Object.keys(updateData).length > 0) {
      const { error } = await supabase.from("bills").update(updateData).eq("id", id).eq("org_id", orgId);
      if (error) throw error;
    }
  }
};

// src/server/banking.ts
var BankingService = class {
  static {
    __name(this, "BankingService");
  }
  static async getTransactions(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("bank_transactions").select("*").eq("org_id", orgId).order("date", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async syncTransactions(orgId) {
    return { count: 0, message: "Bank sync isn't connected yet" };
  }
  static async getRules(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("bank_rules").select("id, match_text, target_account_id, accounts!inner(code, name)").eq("org_id", orgId).order("created_at", { ascending: true });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
  ...
  static async createRule(orgId, matchText, targetAccountId, createdBy) {
    const supabase = getSupabase();
    if (!matchText?.trim()) throw new Error("Match text is required.");
    if (!targetAccountId) throw new Error("A target account is required.");
    const { data, error } = await supabase.from("bank_rules").insert({ org_id: orgId, match_text: matchText.trim(), target_account_id: targetAccountId, created_by: createdBy || null }).select("id").single();
    if (error) throw error;
    return data.id;
  }
  static async deleteRule(orgId, id) {
    const supabase = getSupabase();
    const { error } = await supabase.from("bank_rules").delete().eq("id", id).eq("org_id", orgId);
    if (error) throw error;
  }
  static async requestBankConnection(orgId, input, requestedBy) {
    const supabase = getSupabase();
    if (!input.institutionName?.trim()) throw new Error("Institution name is required.");
    const { data, error } = await supabase.from("bank_connection_requests").insert({
      org_id: orgId,
  ...
  static async getConnectionRequests(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("bank_connection_requests").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
  ...
  static async getReconciliationSummary(orgId) {
    const transactions = await this.getTransactions(orgId);
    const statementBalanceCents = transactions.reduce(
      (sum, tx) => sum + (tx.direction === "IN" ? tx.amountCents : -tx.amountCents),
      0
    );
  ...
  static async getAIMatches(orgId) {
    const transactions = await this.getTransactions(orgId);
    const unreviewed = transactions.filter((t) => t.status !== "MATCHED");
  ...
  static async autoReconcileAll(orgId, minConfidence = 85, userId) {
    const matches = await this.getAIMatches(orgId);
    const qualifying = matches.filter((m) => m.confidence >= minConfidence);
    let reconciledCount = 0;
    for (const match2 of qualifying) {
      try {
        const targetAccount = await AccountService.getAccountByCode(orgId, match2.suggestedAccountCode) || await AccountService.getAccountByCode(orgId, match2.transaction.direction === "IN" ? "4000" : "6000");
        if (targetAccount) {
          await this.matchTransaction(orgId, match2.transactionId, targetAccount.id, void 0, userId);
          reconciledCount++;
        }
      } catch (err) {
  ...
  static async matchTransaction(orgId, transactionId, targetAccountId, existingJournalEntryId, userId) {
    const supabase = getSupabase();
    const { data: tx, error: txError } = await supabase.from("bank_transactions").select("*").eq("org_id", orgId).eq("id", transactionId).single();
    ...
    await supabase.from("bank_transactions").update({
      status: "MATCHED"
    }).eq("org_id", orgId).eq("id", transactionId);
...
var VendorService = class {
  ...
  static async getVendors(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("vendors").select("*").eq("org_id", orgId).order("display_name");
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async createVendor(orgId, input) {
    const supabase = getSupabase();
  ...
  static async updateVendor(orgId, id, input) {
    const supabase = getSupabase();
    const updateData = {};
    if (input.displayName !== void 0) updateData.display_name = input.displayName;
    if (input.legalName !== void 0) updateData.legal_name = input.legalName;
    if (input.vendorType !== void 0) updateData.vendor_type = input.vendorType;
    if (input.contactPerson !== void 0) updateData.contact_person = input.contactPerson;
    if (input.email !== void 0) updateData.email = input.email;
    if (input.phone !== void 0) updateData.phone = input.phone;
    if (input.kraPin !== void 0) updateData.kra_pin = input.kraPin;
    ...
    if (input.country !== void 0) updateData.country = input.country;
    if (input.notes !== void 0) updateData.notes = input.notes;
    const { error } = await supabase.from("vendors").update(updateData).eq("id", id).eq("org_id", orgId);
    if (error) throw error;
  }
};

// src/server/payroll.ts
function calculatePayslip(grossCents) {
...
__name(calculatePayslip, "calculatePayslip");
var PayrollService = class {
  ...
  static async getEmployees(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("employees").select("*").eq("org_id", orgId).order("last_name");
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async addEmployee(orgId, input) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("employees").insert({
      ...
      hire_date: input.hireDate || (/* @__PURE__ */ new Date()).toISOString().substring(0, 10),
  ...
  static async updateEmployee(orgId, id, input) {
    const supabase = getSupabase();
    const updateData = {};
    if (input.firstName !== void 0) updateData.first_name = input.firstName;
    if (input.lastName !== void 0) updateData.last_name = input.lastName;
    if (input.email !== void 0) updateData.email = input.email;
    if (input.phone !== void 0) updateData.phone = input.phone;
    if (input.department !== void 0) updateData.department = input.department;
    if (input.jobTitle !== void 0) updateData.job_title = input.jobTitle;
    if (input.hireDate !== void 0) updateData.hire_date = input.hireDate;
    if (input.baseSalaryCents !== void 0) updateData.base_salary = input.baseSalaryCents;
    if (input.kraPin !== void 0) updateData.kra_pin = input.kraPin;
    if (input.nssfNumber !== void 0) updateData.nssf_number = input.nssfNumber;
    if (input.shifNumber !== void 0) updateData.nhif_number = input.shifNumber;
    if (input.bankName !== void 0) updateData.bank_name = input.bankName;
    if (input.bankAccountNo !== void 0) updateData.bank_account = input.bankAccountNo;
    const { error } = await supabase.from("employees").update(updateData).eq("id", id).eq("org_id", orgId);
    if (error) throw error;
  }
  static async runPayroll(orgId, period, payDate, createdBy) {
    const supabase = getSupabase();
    const { data: employees, error: empError } = await supabase.from("employees").select("id, base_salary, status").eq("org_id", orgId).eq("status", "Active");
    if (empError) throw empError;
    if (!employees || employees.length === 0) throw new Error("No active employees to run payroll for.");
    const { data: existingRun } = await supabase.from("payroll_runs").select("id").eq("org_id", orgId).eq("period", period).maybeSingle();
    ...
    const journalEntryId = await LedgerService.postJournalEntry({
      ...
      lines: [
        ...
        ...vatPayableAccount && totalStatutoryCents > 0 ? [{ accountId: vatPayableAccount.id, debit: 0, credit: totalStatutoryCents, description: `PAYE/NSSF/SHIF/AHL withheld \u2014 ${period}` }] : []
      ]
    });
    ...
    const { error: payslipsError } = await supabase.from("payslips").insert(breakdowns.map((b) => ({
  ...
  static async getPayrollRuns(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("payroll_runs").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      period: row.period,
  ...
  static async getPayslips(orgId, payrollRunId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("payslips").select("*, payroll_runs!inner(org_id), employees(first_name, last_name)").eq("payroll_run_id", payrollRunId).eq("payroll_runs.org_id", orgId);
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
...
var InventoryService = class {
  static {
    __name(this, "InventoryService");
  }
  static async getItems(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("inventory_items").select("*").eq("org_id", orgId).order("name");
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async createItem(orgId, input) {
    const supabase = getSupabase();
  ...
  static async updateItem(orgId, id, input) {
    const supabase = getSupabase();
    const updateData = {};
    if (input.name !== void 0) updateData.name = input.name;
    if (input.itemType !== void 0) updateData.type = input.itemType;
    ...
    if (input.priceCents !== void 0) updateData.unit_price_cents = input.priceCents;
    if (input.costCents !== void 0) updateData.cost_price_cents = input.costCents;
    if (input.incomeAccountId !== void 0) updateData.income_account_id = input.incomeAccountId;
    if (input.expenseAccountId !== void 0) updateData.cogs_account_id = input.expenseAccountId;
    if (input.quantityOnHand !== void 0) updateData.quantity_on_hand = input.quantityOnHand;
    if (input.reorderPoint !== void 0) updateData.reorder_point = input.reorderPoint;
    const { error } = await supabase.from("inventory_items").update(updateData).eq("id", id).eq("org_id", orgId);
    if (error) throw error;
  }
};
...
var ProjectService = class {
  static {
    __name(this, "ProjectService");
  }
  static async getProjects(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("projects").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      orgId: row.org_id,
  ...
  static async createProject(orgId, input) {
    const supabase = getSupabase();
  ...
  static async updateProject(orgId, id, input) {
    const supabase = getSupabase();
    const updateData = {};
    if (input.name !== void 0) updateData.name = input.name;
    if (input.projectCode !== void 0) updateData.project_code = input.projectCode;
    if (input.customerId !== void 0) updateData.customer_id = input.customerId;
    if (input.status !== void 0) updateData.status = input.status;
    if (input.startDate !== void 0) updateData.start_date = input.startDate;
    if (input.endDate !== void 0) updateData.end_date = input.endDate;
    if (input.budgetCents !== void 0) updateData.budget_cents = input.budgetCents;
    if (input.managerId !== void 0) updateData.manager_id = input.managerId;
    const { error } = await supabase.from("projects").update(updateData).eq("id", id).eq("org_id", orgId);
    if (error) throw error;
  }
  static async getTimeEntries(orgId) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("time_entries").select("id, project_id, user_id, entry_date, hours, description, created_at, projects!inner(name)").eq("org_id", orgId).order("entry_date", { ascending: false }).limit(100);
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
  ...
  static async submitTimeEntry(orgId, userId, input) {
    const supabase = getSupabase();
...
// worker/test-minimal.ts
var app = new Hono2();
app.get("/api/ping", (c) => c.json({ ok: true, x: [AccountService, CustomerService, InvoiceService, BankingService, VendorService, BillService, PayrollService, InventoryService, ProjectService, LedgerService, AuditService].length }));
var test_minimal_default = app;
...
// .wrangler/tmp/bundle-mut3WX/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = test_minimal_default;
...
// .wrangler/tmp/bundle-mut3WX/middleware-loader.entry.ts
