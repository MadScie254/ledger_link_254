...
// worker/auth.ts
var writeRoles = /* @__PURE__ */ new Set(["owner", "admin"]);
var UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function isOrganizationCollectionRequest(c) {
  const path = new URL(c.req.url).pathname;
  return path === "/api/organizations" && (c.req.method === "GET" || c.req.method === "POST");
}
__name(isOrganizationCollectionRequest, "isOrganizationCollectionRequest");
async function requireAuthenticationAndOrganization(c, next) {
  ...
  const { data: membership, error: membershipError } = await supabase.from("memberships").select("role").eq("org_id", requestedOrgId).eq("user_id", user.id).maybeSingle();
  if (membershipError) {
...
__name(requireAuthenticationAndOrganization, "requireAuthenticationAndOrganization");
async function requireOrganizationAdministrator(c, next) {
...
__name(requireOrganizationAdministrator, "requireOrganizationAdministrator");
async function requireRequestedOrganization(c, next) {
  if (c.req.param("id") !== c.get("orgId")) {
    return c.json({ error: "The requested organization does not match the active organization." }, 403);
  }
  await next();
}
__name(requireRequestedOrganization, "requireRequestedOrganization");

// worker/test-minimal.ts
var app = new Hono2();
app.use("*", cors());
app.get("/api/ping", (c) => c.json({ ok: true, x: [requireAuthenticationAndOrganization, requireOrganizationAdministrator, requireRequestedOrganization].length }));
...
// .wrangler/tmp/bundle-fjXefP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fjXefP/middleware-loader.entry.ts
