...
// .wrangler/tmp/bundle-2JTbfo/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-2JTbfo/middleware-insertion-facade.js
init_modules_watch_stub();
...
var encodeSimple = /* @__PURE__ */ __name((key, value, options) => {
  let out = "";
  const pairs = (options === null || options === void 0 ? void 0 : options.explode) ? explode(key, value) : [[key, value]];
  if (pairs.every(([_, v]) => v == null)) {
    return;
...
var DEFAULT_FETCHER = /* @__PURE__ */ __name((input, init) => {
  if (init == null) {
    return fetch(input);
  } else {
  ...
  constructor(responseBody, parse, opts) {
    ...
    super({
      async pull(downstream) {
        try {
          while (true) {
            const match3 = findBoundary(buffer, bufferLen, searchStart);
            if (!match3) {
            ...
            const message = buffer.slice(0, match3.index);
            buffer.copyWithin(0, match3.index + match3.length, bufferLen);
            bufferLen -= match3.index + match3.length;
  ...
  return /* @__PURE__ */ __name(async function matchFunc(response, request, options) {
    ...
    switch (encoding) {
      ...
      case "jsonl":
        ...
        break;
      case "bytes":
        raw2 = new Uint8Array(await response.arrayBuffer());
        break;
      case "stream":
        raw2 = response.body;
        break;
      ...
      case "sse":
        if (response.body) {
          ...
          raw2 = wrapEventStreamResponse(response.body, sseOpts);
        } else {
          raw2 = null;
        }
        break;
    ...
    if ("err" in matcher) {
      data = Object.assign(Object.assign(Object.assign({}, options === null || options === void 0 ? void 0 : options.extraFields), headersField), isPlainObject$1(raw2) ? raw2 : null);
    } else if (resultKey) {
      data = Object.assign(Object.assign(Object.assign({}, options === null || options === void 0 ? void 0 : options.extraFields), headersField), { [resultKey]: raw2 });
    ...
    if ("err" in matcher) {
      ...
      return [{ ok: false, error: errValue }, raw2];
    }
    return [{ ok: true, value: data }, raw2];
  }, "matchFunc");
}
__name(match2, "match");
var headerValRE = /, */;
...
async function $do$q(client, body, api_version, options) {
  ...
  const [result] = await match2(fail("4XX"), fail("5XX"), json("default"))(response, req);
  if (!result.ok) {
...
__name(environmentsFilesList, "environmentsFilesList");
async function $do$i(client, environment, path, api_version, page_size, page_token, recursive, options) {
  ...
  const response = doResult.value;
  const [result] = await match2(json(200), fail("4XX"), fail("5XX"))(response, req);
  if (!result.ok) {
    return [result, { status: "complete", request: req, response }];
  }
  return [result, { status: "complete", request: req, response }];
...
async function $do$h(client, id, api_version, options) {
  ...
  const [result] = await match2(json(200), jsonErr("4XX", CancelInteractionByIdClientError), jsonErr("5XX", CancelInteractionByIdServerError))(response, req, { extraFields: responseFields });
  if (!result.ok) {
    return [result, { status: "complete", request: req, response }];
  }
  return [result, { status: "complete", request: req, response }];
...
async function $do$g(client, body, api_version, options) {
  ...
  const [result] = await match2(json(200), sse(200, {
    sentinel: "[DONE]",
    flattened: true
...
async function $do$f(client, id, api_version, options) {
  ...
  const [result] = await match2(nil(200), jsonErr("4XX", DeleteInteractionClientError), jsonErr("5XX", DeleteInteractionServerError))(response, req, { extraFields: responseFields });
  if (!result.ok) {
    return [result, { status: "complete", request: req, response }];
  }
  return [result, { status: "complete", request: req, response }];
...
var GeminiNextGenEnvironmentFiles = class {
  static {
    __name(this, "GeminiNextGenEnvironmentFiles");
  }
...
var ai = new GoogleGenAI2({ apiKey: apiKey || "dummy-key" });
var GeminiService = class {
  static {
    __name(this, "GeminiService");
  }
  static async scanReceipt(base64Image, mimeType) {
    if (!apiKey) {
...
// worker/test-minimal.ts
var app = new Hono2();
app.get("/api/ping", (c) => c.json({ ok: true, hasService: !!GeminiService }));
